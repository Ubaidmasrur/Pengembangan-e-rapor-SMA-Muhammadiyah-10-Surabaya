<x-app-layout>
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <h1 class="text-2xl font-bold mb-4">Tambah Wali Murid</h1>

        <form method="POST" action="{{ route('admin.guardians.store') }}" class="space-y-4">
            @csrf
                ...
                <select name="relationship" id="relationship"
                    ...
                    @foreach (['Orang Tua', 'Saudara'] as $relationship)
                        <option value="{{ $relationship }}"
                            {{ old('relationship') == $relationship ? 'selected' : '' }}>
                            {{ $relationship }}</option>
                    @endforeach
                </select>
                @error('relationship')
