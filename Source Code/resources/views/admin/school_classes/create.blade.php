<x-app-layout>
    ...
    <main>
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <h1 class="text-2xl font-bold mb-4" id="form-title">Tambah Kelas</h1>
            ...
            <section aria-labelledby="form-title">
                <form id="school-class-form" role="form" aria-labelledby="form-title" method="POST"
                    action="{{ route('admin.school_classes.store') }}" class="space-y-4">
                    @csrf

                    {{-- Sekolah --}}
                    <x-form-select name="school_id" label="Sekolah" :options="$schools->pluck('name', 'id')" :value="old('school_id')"
                        placeholder="-- Pilih Sekolah --" aria-label="Sekolah" />

                    {{-- Nama Kelas --}}
                    <x-form-input name="name" label="Nama Kelas" type="text" :value="old('name')"
