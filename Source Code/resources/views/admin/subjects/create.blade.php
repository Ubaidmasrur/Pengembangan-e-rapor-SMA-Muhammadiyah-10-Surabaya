<x-app-layout>
    ...
    <main>
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <h1 class="text-2xl font-bold mb-4" id="form-title">Tambah Mata Pelajaran</h1>

            <section aria-labelledby="form-title">
                <form id="subject-form" role="form" aria-labelledby="form-title" method="POST"
                    action="{{ route('admin.subjects.store') }}" class="space-y-4">
                    @csrf

                    {{-- Nama Mapel --}}
                    <x-form-input name="name" label="Nama Mapel" :value="old('name')" />

                    {{-- Tipe --}}
                    <x-form-select name="type" label="Tipe" :options="['umum' => 'Umum', 'khusus' => 'Khusus', 'ekstra' => 'Ekstra']" :value="old('type')" />
