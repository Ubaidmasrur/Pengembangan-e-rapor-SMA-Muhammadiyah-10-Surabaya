<x-app-layout>
    <main class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <header class="mb-6">
                <h1 class="font-semibold text-xl text-gray-800 leading-tight">Daftar Laporan Hasil Belajar</h1>
            </header>

            <section class="bg-white rounded-lg shadow p-4 mb-6">
                <form method="GET" action="{{ route('admin.report.admin') }}"
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <a href="{{ route('admin.report.admin') }}"
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-700">
                                        <div class="flex items-center gap-2">
                                            @if (!empty($report->has_report))
                                                <a href="{{ route('admin.report.preview', $report->sample_id) }}"
                                                    target="_blank"
                                                    class="px-3 py-1 bg-green-600 text-white rounded text-xs">Lihat
                                                    Rapor</a>
                                                <a href="{{ route('admin.report.export', $report->sample_id) }}"
