        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-lg p-6">
                <div class="flex items-start justify-between mb-4">
                    <div>
                        <h1 class="text-xl font-bold">Laporan Akademik</h1>
                        <div class="text-sm text-gray-600">Ringkasan nilai dan catatan perkembangan</div>
                    </div>
                    <div class="text-right">
                        <div class="font-semibold">{{ optional($student)->name ?? 'Siswa' }}</div>
                        <div class="text-sm text-gray-600">
                            {{ optional($student)->class_name ?? (optional(optional($student)->class)->name ?? '') }}
                        </div>
                    </div>
                </div>

                @if ($masters->isEmpty())
                    <div class="p-6 bg-yellow-50 border border-yellow-200 rounded text-yellow-800">Belum ada data nilai.
                    </div>
                @else
                    <div class="space-y-4">
                            ...
                            <div class="border rounded-lg p-4">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <div class="font-semibold">{{ optional($m->academicYear)->year ?? 'Tahun' }} -
                                            {{ optional($m->academicYear)->semester ?? '' }}</div>
                                    </div>
                                    <div>
                                        <div class="text-sm text-gray-500">Periode</div>
                                        <div class="font-semibold">{{ $m->period ?? '' }}</div>
                                    </div>
                                    ...
                                    <div class="text-right">
                                        <button data-toggle-detail="master-{{ $m->id }}"
                                            class="px-3 py-2 bg-indigo-600 text-white rounded text-sm toggle-detail">Lihat
                                            Rincian</button>
                                    </div>
                                </div>
                                    ...
                                    <div class="overflow-x-auto">
                                        <table class="min-w-full divide-y divide-gray-200">
                                            <thead class="bg-gray-50">
                                                <tr>
                                                    <th class="px-4 py-2 text-left text-xs text-gray-500">Mata Pelajaran
                                                    </th>
                                            ...
                                            <tbody class="bg-white divide-y divide-gray-200">
                                                    ...
                                                    @foreach ($m->details as $d)
                                                        <tr>
                                                            <td class="px-4 py-2 text-sm text-gray-700">
                                                                {{ optional($d->subject)->name ?? ($d->subject_name ?? '-') }}
                                                            </td>
                                                            <td class="px-4 py-2 text-sm text-gray-700">
                                                                {{ $m->period ?? '-' }}
                                                            </td>
                                                ...
                                                @else
                                                    <tr>
                                                        <td colspan="5" class="px-4 py-2 text-sm text-gray-500">Belum
                                                            ada rincian.</td>
                                                    </tr>
                                                @endif
                                            </tbody>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                document.querySelectorAll('.toggle-detail').forEach(function(btn) {
                    btn.addEventListener('click', function() {
                        ...
                        this.textContent = el.classList.contains('hidden') ? 'Lihat Rincian' :
                            'Sembunyikan';
                    });
                });
